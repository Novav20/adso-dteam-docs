...
> La grilla base, tipografía, paletas neutras y áreas de contacto táctil mínimas se heredan de [[DT-UI-DS-DOC-001|DT-UI-DS-DOC-001]]. Esta tabla define exclusivamente los componentes presentes, sus tokens semánticos y el enlace con el modelo.
...
| `CMP-04` | Equipment Hotspot       | Símbolo de equipo en SVG         | `--dt-color-surface-card`                                                                                                        | Geometría de Nivel 6 vinculada unívocamente por `TagNumber` (FR-599). En condición normal opera en gris neutro; en alarma adquiere halo y forma de severidad según [[DT-UI-DS-DOC-001\|DT-UI-DS-DOC-001]]. |
...
| `CMP-06` | Asset Header Block      | Identificación y estado          | `--dt-font-mono-data`                                                                                                            | Presenta `TagNumber`, criticidad para priorización ([[ADR-002\|ADR-002]]) y estado operativo según `EquipmentUnit.operationalStatus` ([[DT-DM-DOC-001\|DT-DM-DOC-001]]).     |
| `CMP-07` | Live Telemetry Block    | Indicadores analógicos MAI       | `--dt-color-mai-*`                                                                                                               | Barras analógicas para variables de proceso críticas ([[DT-UI-DS-DOC-001\|DT-UI-DS-DOC-001]]). Se actualiza dinámicamente vía SignalR ([[TR-010]]).                                                        |
...
3. **Responsive Layout:** La transformación del contenedor `CMP-05` (Bottom Sheet $\leftrightarrow$ Lateral Panel) se delega a las reglas de orientación de dispositivo definidas en [[DT-UI-NAV-DOC-001|DT-UI-NAV-DOC-001]].
...
2. **Gestión de Suscripción SignalR:** Sujeta a políticas globales de *debouncing* para prevenir colisiones de red por selección rápida múltiple ([[DT-ARQ-CMP-DOC-001|DT-ARQ-CMP-DOC-001]]).
3. **Resiliencia de Conexión:** Si el componente detecta violación del umbral del *Heartbeat* o recibe un paquete con calidad "Bad" ([[DT-ARQ-DEP-DOC-001|DT-ARQ-DEP-DOC-001]]), transiciona inmediatamente a la visualización de "Pérdida de Telemetría".
---
