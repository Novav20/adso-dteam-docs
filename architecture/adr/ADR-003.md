...
4. **Captura Atómica de Estados:** Para poblar los atributos `previous_state` y `new_state` sin incurrir en lecturas previas (`SELECT`) que penalicen la latencia, las mutaciones transaccionales aprovecharán la capacidad nativa de PostgreSQL 18 para proyectar estados atómicos mediante la cláusula `RETURNING old.*, new.*`. Esto asegura que la captura de auditoría se resuelva en el mismo viaje de red de la transacción principal (<50ms, conforme a TR-001-NFR-366).
