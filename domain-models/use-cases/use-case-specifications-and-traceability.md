...
version: 1.1
date: 2026-07-16
status: Matriz de Trazabilidad de Casos de USO
...
| :--------------- | :------------------------------------------ | :----------------------- | :------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------- |
| **UC-MTTO-001**  | Programar Mantenimiento Preventivo (Tiempo) | [[MTTO-001]]             | Planificador               | **ISO 55001 (8.1):** Programación sistemática para evitar el desgaste acelerado y fallas catastróficas.                                    |
...
| **UC-MTTO-026**  | Priorizar Backlog mediante RIME             | [[MTTO-026]]             | Planificador               | **ISO 55001 (6.2.2):** Priorización objetiva de recursos críticos: $Score = \text{Criticidad} \times \text{Clase de Trabajo}$.             |
| **UC-MTTO-002**  | Cerrar OT con Taxonomía ISO 14224           | [[MTTO-002]]             | Técnico                    | **ISO 14224 (Sección 5):** Captura mandatoria de datos de falla (modo, causa y mecanismo) para indicadores de fiabilidad (MTBF).           |
...
| **UC-MTTO-029**  | Definir Límites de Activos (Boundaries)     | [[MTTO-029]]             | Ingeniero de Confiabilidad | **ISO 14224 (Sección 5.6):** Especificación mandatoria de los límites del equipo para evitar duplicidad de costos de mantenimiento.        |
...
| **UC-INV-021**   | Monitor de Reabastecimiento             | [[INV-021]]              | Jefe de Almacén            | **ISO 55001 (7.5 - Planificación):** Gestión de compras técnicas automatizadas basadas en criticidad y repuestos clase A, B o C.                              |
...
| **UC-INV-025**   | Rotación de Activos (Asset Swap)        | [[INV-025]]              | Ingeniero de Proyectos     | **ISO 14224 (Sección 6 - Taxonomía):** Desacoplamiento físico-lógico: mueve el activo desmontado al almacén y vincula el reemplazo en la ubicación funcional. |
...
| **UC-VIS-009**   | Navegación Jerárquica y Zoom Semántico         | [[VIS-009]]              | Supervisor             | **HPHMI (Hollifield):** Estructura jerárquica de navegación por niveles (1-3) para mitigar fatiga de datos (UX).                               |
| **UC-VIS-010**   | Visualización de Despiece (Vista Explosionada) | [[VIS-010]]              | Técnico                | **ISO 55001 (7.5 - Accesibilidad):** Despiece en 3D para reconocimiento espacial previo al desmontaje físico.                                  |
| **UC-VIS-011**   | Rutas de Aislamiento (LOTO) y Bloqueo Activo   | [[VIS-011]]              | Técnico                | **ISO 45001 (8.1) / ASR-2:** Bloqueo de software (fail-safe) que impide iniciar la ejecución si hay energía en campo o falla el latido de red. |
| **UC-VIS-012**   | Configuración de Jerarquía de Componentes 3D   | [[VIS-012]]              | Ingeniero de Proyectos | **ISO 55001 (7.5):** Administración técnica de la correspondencia entre mallas 3D (`.glb`) y el árbol taxonómico relacional.                   |
...
| **UC-ADM-014**   | Gestionar Usuarios y Ciclo de Vida    | [[ADM-014]]              | Administrador         | **ISO 27001 (A.8.2 - IAM):** Ciclo seguro: inactividad por fuerza bruta, cifrado PBKDF2 y borrado lógico (soft-delete).                  |
...
| **UC-ADM-017**   | Dashboard Ejecutivo de KPIs           | [[ADM-017]]              | Gerente de Planta     | **ISO 14224 (Anexo C):** Consolidación matemática de MTBF, MTTR e indisponibilidad para identificar "activos problema" (Pareto).         |
...
| **UC-ADM-022**   | Sugerencias de Optimización (PMO)     | [[ADM-022]]              | Ing. de Confiabilidad | **ISO 55001 (10.2 - Mejora Continua):** Análisis histórico de fallas para evitar sobre-mantenimiento (CAPEX/OPEX).                       |
...
| **UC-ADM-032**   | Sistema de Auditoría Inmutable        | [[ADM-032]]              | Auditor / Gerente     | **ISO 27001 (A.8.15 - Registro) / ADR-003:** Trazabilidad criptográfica SHA-256 encadenada para asegurar la inalterabilidad de los logs. |
