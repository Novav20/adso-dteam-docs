...
| `InstallEquipment(equipment: EquipmentUnit): void` | Installs an asset in the operational position, changing its status to INSTALLED and formally generating a new `AssetInstallation` history record. | [[INV-025]]<br>FR-184, FR-185 | **Business Invariant:** Validates that the position is not occupied, the equipment is operational, and explicitly records the domain timeline for MTBF tracking. |
