...
| `InstallEquipment(equipment: EquipmentUnit): void` | Installs an asset in the operational position, changing its status to INSTALLED and formally generating a new `AssetInstallation` history record. | [[INV-025]]<br>FR-184, FR-185 | **Business Invariant:** Validates that the position is not occupied, the equipment is operational, and explicitly records the domain timeline for MTBF tracking (F-09). |
| `UninstallEquipment(): EquipmentUnit` | Removes the asset from the operational location, closing (setting `removed_at`) the current `AssetInstallation` history record and returning the equipment. | [[INV-025]]<br>FR-183 | **State Machine:** Breaks the link, secures the timeline evidence, and enables the equipment for physical relocation to the warehouse. |
